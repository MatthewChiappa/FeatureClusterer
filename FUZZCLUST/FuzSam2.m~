...
np_x_1  = ones(np, 1); % ones vector of w/ the number of points
nc_x_1  = ones(nc, 1); % ones vector of clusters
odim_x_1 = ones(odim,1); % ones matrix of partition matrix
...
% initializing   
x  = P ; % projection matrix
fm = f.^m; % partition matrix to the weighting exponent
sumf = sum(fm); % sum of the columns of fm
xv = (fm'*x)./(sumf'*ones(1,2));   %projected cluster centers

c = sum(x) / np;    % move the center of mass to the center
x = x - c(np_x_1, :); 
xv = xv - c(nc_x_1, :); % projected cluster centers
